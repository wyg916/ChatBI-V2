...
def new_cases() -> list[dict[str, Any]]:
    return [
        ...
        case("G26", "group", "按地区统计订单成本", ["orders", "regions"], ["cost"], ["region"],
             "SELECT r.region_name AS region, SUM(o.cost) AS cost FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id GROUP BY r.region_name ORDER BY cost DESC"),
        case("G27", "topn_join", "按产品统计订单利润前3名", ["orders", "products"], ["profit"], ["product"],
             "SELECT p.product_name AS product, SUM(o.revenue - o.cost) AS profit FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id GROUP BY p.product_name ORDER BY profit DESC LIMIT 3"),
        case("G28", "group_join", "按品类统计订单收入", ["orders", "products"], ["revenue"], ["category"],
             "SELECT p.category AS category, SUM(o.revenue) AS revenue FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id GROUP BY p.category ORDER BY revenue DESC"),
        case("G29", "topn_join", "按客户统计订单收入前5名", ["orders", "customers"], ["revenue"], ["customer"],
             "SELECT c.customer_name AS customer, SUM(o.revenue) AS revenue FROM demo_business.orders o JOIN demo_business.customers c ON c.customer_id = o.customer_id GROUP BY c.customer_name ORDER BY revenue DESC LIMIT 5"),
        case("G30", "group_join", "按客户类型统计订单量", ["orders", "customers"], ["order_count"], ["customer_type"],
             "SELECT c.customer_type AS customer_type, COUNT(o.order_id) AS order_count FROM demo_business.orders o JOIN demo_business.customers c ON c.customer_id = o.customer_id GROUP BY c.customer_type ORDER BY order_count DESC"),
        ...
        case("G32", "multi_dimension_time", "2026年按地区按月统计订单成本", ["orders", "regions"], ["cost"], ["region", "month"],
             "SELECT r.region_name AS region, DATE_TRUNC('month', o.order_date) AS month, SUM(o.cost) AS cost FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id WHERE o.order_date >= DATE '2026-01-01' AND o.order_date < DATE '2027-01-01' GROUP BY r.region_name, DATE_TRUNC('month', o.order_date) ORDER BY month ASC, cost DESC",
             time_range={"start": "2026-01-01", "end_exclusive": "2027-01-01"}),
        case("G33", "filter_join", "按地区统计已支付订单成本", ["orders", "regions"], ["cost"], ["region"],
             "SELECT r.region_name AS region, SUM(o.cost) AS cost FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id WHERE o.status = 'PAID' GROUP BY r.region_name ORDER BY cost DESC",
             filters=[{"field": "orders.status", "value": "PAID"}]),
        case("G34", "filter_join", "按品类统计已退款订单量", ["orders", "products"], ["order_count"], ["category"],
             "SELECT p.category AS category, COUNT(o.order_id) AS order_count FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id WHERE o.status = 'REFUNDED' GROUP BY p.category ORDER BY order_count DESC",
             filters=[{"field": "orders.status", "value": "REFUNDED"}]),
        case("G35", "multi_filter", "统计华东地区订单收入", ["orders", "regions"], ["revenue"], ["region"],
             "SELECT r.region_name AS region, SUM(o.revenue) AS revenue FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id WHERE r.region_name = '华东' GROUP BY r.region_name ORDER BY revenue DESC",
             filters=[{"field": "regions.region_name", "value": "华东"}]),
        case("G36", "multi_filter", "统计华南地区已支付订单利润", ["orders", "regions"], ["profit"], ["region"],
             "SELECT r.region_name AS region, SUM(o.revenue - o.cost) AS profit FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id WHERE o.status = 'PAID' AND r.region_name = '华南' GROUP BY r.region_name ORDER BY profit DESC",
        ...
        case("G37", "category_filter", "按品类统计充电设备收入", ["orders", "products"], ["revenue"], ["category"],
             "SELECT p.category AS category, SUM(o.revenue) AS revenue FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id WHERE p.category = '充电设备' GROUP BY p.category ORDER BY revenue DESC",
             filters=[{"field": "products.category", "value": "充电设备"}]),
        case("G38", "category_filter", "按品类统计储能设备已退款订单成本", ["orders", "products"], ["cost"], ["category"],
             "SELECT p.category AS category, SUM(o.cost) AS cost FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id WHERE o.status = 'REFUNDED' AND p.category = '储能设备' GROUP BY p.category ORDER BY cost DESC",
        ...
        case("G39", "complex_join", "按地区按品类统计订单收入", ["orders", "regions", "products"], ["revenue"], ["region", "category"],
             "SELECT r.region_name AS region, p.category AS category, SUM(o.revenue) AS revenue FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id JOIN demo_business.products p ON p.product_id = o.product_id GROUP BY r.region_name, p.category ORDER BY revenue DESC"),
        case("G40", "multi_dimension", "按产品按状态统计订单量", ["orders", "products"], ["order_count"], ["product", "status"],
             "SELECT p.product_name AS product, o.status AS status, COUNT(o.order_id) AS order_count FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id GROUP BY p.product_name, o.status ORDER BY order_count DESC"),
        case("G41", "multi_dimension", "按客户类型按状态统计订单收入", ["orders", "customers"], ["revenue"], ["customer_type", "status"],
             "SELECT c.customer_type AS customer_type, o.status AS status, SUM(o.revenue) AS revenue FROM demo_business.orders o JOIN demo_business.customers c ON c.customer_id = o.customer_id GROUP BY c.customer_type, o.status ORDER BY revenue DESC"),
        ...
        case("G43", "contribution", "按地区统计收入贡献度", ["orders", "regions"], ["revenue_share"], ["region"],
             "SELECT r.region_name AS region, ROUND(SUM(o.revenue) * 100.0 / NULLIF(SUM(SUM(o.revenue)) OVER (), 0), 4) AS revenue_share FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id GROUP BY r.region_name ORDER BY revenue_share DESC"),
        case("G44", "ratio_zero_safe", "按品类统计利润率", ["orders", "products"], ["profit_margin"], ["category"],
             "SELECT p.category AS category, ROUND(SUM(o.revenue - o.cost) * 100.0 / NULLIF(SUM(o.revenue), 0), 4) AS profit_margin FROM demo_business.orders o JOIN demo_business.products p ON p.product_id = o.product_id GROUP BY p.category ORDER BY profit_margin DESC"),
        ...
        case("G47", "duplicate_grain", "按客户统计去重订单量前5名", ["orders", "customers"], ["distinct_order_count"], ["customer"],
             "SELECT c.customer_name AS customer, COUNT(DISTINCT o.order_id) AS distinct_order_count FROM demo_business.orders o JOIN demo_business.customers c ON c.customer_id = o.customer_id GROUP BY c.customer_name ORDER BY distinct_order_count DESC LIMIT 5"),
        case("G48", "quarter_boundary", "2026年第一季度按地区统计收入和成本", ["orders", "regions"], ["revenue", "cost"], ["region"],
             "SELECT r.region_name AS region, SUM(o.revenue) AS revenue, SUM(o.cost) AS cost FROM demo_business.orders o JOIN demo_business.regions r ON r.region_id = o.region_id WHERE o.order_date >= DATE '2026-01-01' AND o.order_date < DATE '2026-04-01' GROUP BY r.region_name ORDER BY revenue DESC",
