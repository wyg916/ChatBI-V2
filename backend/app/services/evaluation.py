...
MULTI_GROUND_TRUTH_PATH = _SERVICE_FILE.parents[3] / "evaluation" / "golden" / "v2.1-multiple-ground-truth.json"
MULTI_GROUND_TRUTH_SHA256 = "c41971b86516e89b69e0d2b1ffbee9d9c9a8aecae5cf4017dd06295555595df0"
RELEASE_THRESHOLDS = {
    "golden_count": 50,
...
def load_multiple_ground_truth() -> dict[str, list[str]]:
    ...
    if hashlib.sha256(payload).hexdigest() != MULTI_GROUND_TRUTH_SHA256:
...
def create_evaluation(db: Session, *, name: str, profile: dict[str, Any]) -> EvaluationRun:
    manifest = load_golden_manifest()
    resolved_profile = {**DEFAULT_PROFILE, **profile}
    run = EvaluationRun(
        workspace_id=default_workspace(db).id,
        release_name=name,
...
def run_golden_evaluation(
    ...
    *,
    ...
    if run_id:
        ...
        run = EvaluationRun(
            workspace_id=workspace.id, release_name="ChatBI V2.1 Golden 50",
...
def compare_evaluation_runs(db: Session, run_ids: list[str]) -> dict[str, Any]:
...
def release_gate(db: Session, run_id: str) -> dict[str, Any]:
    run, _ = evaluation_run_detail(db, run_id)
    total = max(run.golden_set_count, 1)
    metrics = {
...
def evaluation_dashboard(db: Session, run_id: str | None = None) -> dict[str, Any]:
    if run_id:
        run, cases = evaluation_run_detail(db, run_id)
    else:
        run = db.scalar(select(EvaluationRun).where(EvaluationRun.is_current.is_(True)).order_by(EvaluationRun.completed_at.desc()))
        if run is None:
    ...
    return {
        ...
        "release_gate": release_gate(db, run.id),
