...
def get_evaluation_case(case_ref: str, db: Session = Depends(get_db)):
    try:
        ...
        return {"run": evaluation_run_view(run, evaluation_run_detail(db, run.id)[1]), "case": case, "previous_case_id": previous_id, "next_case_id": next_id}
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def create_evaluation_definition(
    ...
    try:
        run = create_evaluation(db, name=data.name, profile=data.profile.model_dump())
...
def execute_evaluation_definition(
    ...
    try:
        run = run_golden_evaluation(db, run_id=run_id)
        _, cases = evaluation_run_detail(db, run.id)
...
def compare_runs(
    ...
    db: Session = Depends(get_db),
    _: Principal = Depends(require_permission("evaluation.read")),
):
    try:
        return compare_evaluation_runs(db, data.run_ids)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def get_evaluation_dashboard(
    ...
    try:
        return evaluation_dashboard(db, run_id)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
def incorrect_feedback(
    ...
    try:
        return submit_correction(db, data=data, workspace_id=principal.workspace_id)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
def replay_feedback(
    ...
    try:
        return replay_verified_sql(db, answer_id=answer_id, data=data, workspace_id=principal.workspace_id)
    except LookupError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
