...
Phase 3 adds two deliberately narrow third-party runtime boundaries plus one separately identified owner-attested original contribution. DB-GPT is installed only from the exact `packages/dbgpt-core` revision and executes AWEL; its application, RAG, datasource, auth, conversation, model-key and skill surfaces are excluded. PandasAI contributes exactly one byte-identical MIT community file, `pandasai/sandbox/sandbox.py`; no root `pandasai` import and no `ee/**` path is packaged. The Legacy RAG owner attests original authorship and contributes only the three checksum-locked selected files for public distribution under Apache-2.0; the remainder of the source project is excluded.
...
| Owner-attested original Legacy RAG contribution | `b2573a9dc1881a54581c5c556fb4a8c34046f9c3` | `backend/app/knowledge/indexer.py`, `reranker.py`, `security.py` | Apache-2.0, limited to the three owner-contributed checksum-locked files | Three byte-identical Git blobs provide the real deterministic index/vector, BM25/RRF/rerank and prompt-injection runtime. ChatBI retains HMAC identity, Workspace/RBAC/ACL, Citation, the single Model Gateway, Answer Guard, Trace/SSE and all data models. Checksums, the owner contribution notice and rollback are locked in `backend/vendor/legacy_energy_rag/.../`. `VENDORED_RUNTIME` is only an integration topology, not a distinct internal/public license class. |
...
- Source identity: owner-attested legacy project locked at `b2573a9dc1881a54581c5c556fb4a8c34046f9c3`. The owner attests that the three checksum-locked selected files are an original contribution and contributes them under Apache-2.0; no absolute source path or other legacy-project content is distributed.
...
- Exact selected paths, Git blobs, canonical SHA-256 values, license notice, dependency/secret/data audit and rollback are in `docs/runtime/V1_3_PHASE3_OWNER_AUTHORIZED_LEGACY_RAG_LOCK.md` and `backend/vendor/legacy_energy_rag/.../`.
