...
The current unreleased working-tree successor declares migration head `20260829_0015`, with direct downgrade target `20260829_0014`, and creates V3 backup manifests. These changes are not part of the immutable `chatbi-v2-v1.3.1` tag and are not certified by the tag's historical test totals.

Migration `0015` fails closed while detached SQL-workspace history exists because `0014` cannot represent a missing datasource link; restore the referenced datasource or retain `0015`. Migration `0014` separately fails closed while managed Excel/CSV datasource rows, schemas or helper readers remain. Export and delete those datasources through the Backend API before an `0014 → 0013` downgrade. A rollback that changes metadata schema must use a verified pre-upgrade backup and the exact project-scoped deployment commands. Do not use `docker compose down -v`, delete local PostgreSQL/MySQL databases, or operate an unrelated Compose project/schema.
...
4. For the unpublished successor, first resolve any detached history before validating `0015 → 0014`; then remove managed spreadsheet sources through the Backend API and validate `0014 → 0013` before any older rollback. For the published V1.3.1 contract, use the audited `0013 → 0012` path in the exact project-owned PostgreSQL schema.
