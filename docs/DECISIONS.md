...
- Derived contracts must advance together: Multiple Ground Truth binds to the new Golden manifest hash, Backend pins the canonical Golden and overlay hashes, and Data100 updates its base hash plus any affected extension expectations and its own manifest hash.
- A fresh-seed IBM mismatch remains a hard release failure even when every SQL request succeeds and the aggregate accuracy threshold would otherwise pass. The failure must be diagnosed from the uploaded exact-SHA artifact; rerunning does not substitute for repairing deterministic seed/Golden drift.
- Tag and Release creation remain blocked after any candidate HEAD change. The repaired candidate requires a new approval, PR gates, and all eight exact-main push jobs, including the IBM self-contained execution comparison.
