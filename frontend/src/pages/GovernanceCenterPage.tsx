...
function TraceView() {
  ...
  return <><Coverage value={result.data.coverage} /><section className="governance-card governance-wide"><div className="governance-card-heading"><h2>ONE_TRACE 时序索引</h2><span>{result.data.trace_granularity}</span></div><div className="governance-table-scroll"><table><thead><tr><th>Trace ID</th><th>路由</th><th>状态</th><th>阶段</th><th>耗时</th><th>Provider / Model</th><th>能力 / Artifact</th><th>错误</th></tr></thead><tbody>{result.data.items.map((item) => <tr key={item.trace_id}><td><button aria-label="查看 TRACE 详情" className="governance-trace-link" data-logical-control="governance.trace.open-detail" data-resource-id={item.trace_id} data-resource-type="TRACE" type="button" onClick={() => setSelectedTraceId(item.trace_id)}><code>{item.trace_id}</code></button></td><td>{item.route ?? 'UNKNOWN'}</td><td>{item.status}</td><td>{item.stage_count}</td><td>{item.duration_ms} ms</td><td>{item.provider ?? '—'} / {item.model ?? '—'}</td><td>{[item.has_sql && 'SQL', item.has_rag && 'RAG', item.has_agent && 'AGENT', item.has_file && 'FILE', item.has_vision && 'VISION'].filter(Boolean).join(' · ') || '—'} · {item.artifact_count}</td><td>{item.error_code ?? '—'}</td></tr>)}</tbody></table></div></section>
