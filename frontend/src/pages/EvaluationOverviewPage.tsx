...
export function EvaluationOverviewPage() {
  ...
  const trendOption = useMemo<EChartsCoreOption>(() => ({
    ...
    yAxis: { type: 'value', min: 0, max: 100, interval: 20, axisLabel: { color: '#75829a', fontSize: 10, formatter: '{value}%' }, splitLine: { lineStyle: { color: '#e7ebf3' } }, axisLine: { show: false }, axisTick: { show: false } },
  ...
  if (!data) return <div className="evaluation-page">
    <h1>评测中心</h1>
    <p>评测记录为空或暂不可用。配置业务评测数据集后可运行评测。</p>
    {result.error && <ErrorNotice error={result.error} />}
    {notice && <p role="status">{notice}</p>}
    <button className="button primary" disabled={runGolden.isPending} onClick={() => runGolden.mutate()}>
      {runGolden.isPending ? '正在运行' : '运行评测'}
    </button>
  </div>;
