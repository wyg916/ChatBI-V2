...
function Test-CanonicalImages {
  foreach ($image in 'chatbi-v2-backend:latest', 'chatbi-v2-frontend:latest', 'chatbi-sandbox-runtime:phase3') {
...
function Invoke-CanonicalBuild {
  ...
  & docker build --progress=plain -t chatbi-v2-backend:latest -f backend/Dockerfile .
  if ($LASTEXITCODE -ne 0) { throw 'Backend image build failed.' }
  Write-Stage 'Building Sandbox image...'
  & docker build --progress=plain -t chatbi-sandbox-runtime:phase3 -f sandbox_runtime/Dockerfile .
  if ($LASTEXITCODE -ne 0) { throw 'Sandbox image build failed.' }
  Write-Stage 'Building Frontend image...'
  & docker build --progress=plain -t chatbi-v2-frontend:latest frontend
...
function Reset-MetadataSchema {
  ...
  & docker compose run --rm --no-deps backend python -m app.showcase.rebuild_schema --confirm-local-showcase-schema-rebuild
...
function Set-DocumentedCredentials {
  ...
  & docker compose exec -T backend python -m app.showcase.reset --credentials-only
...
switch ($Action) {
  ...
  'Reset' {
    ...
    Invoke-StartFromImages
    & docker compose exec -T backend python -m app.showcase.reset --confirm-local-showcase-reset
    if ($LASTEXITCODE -ne 0) { throw 'Local showcase metadata reset failed.' }
    & docker compose restart backend rag-runtime | Out-Host
    Wait-ShowcaseReady
    & (Join-Path $PSScriptRoot 'verify.ps1')
